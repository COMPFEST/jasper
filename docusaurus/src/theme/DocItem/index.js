    ...
    <ul className={isChild ? '' : 'contents contents__left-border'}>
      {headings.map((heading) => (
        <li key={heading.id}>
          <a
            href={`#${heading.id}`}
            className={LINK_CLASS_NAME + `${isChild ? ' link__child' : ' link__parent'}`}
...
function DocItem(props) {
  ...
  let label = title.split("/")[0].toUpperCase();
  let docsTitle = sidebar_label || title;

  const metaTitle = docsTitle ? `${docsTitle} | ${siteTitle}` : siteTitle;
          ...
          <div className={classnames('col', styles.docItemCol)}>
            <div className={styles.docItemContainer}>
              <article>
                ...
                {!hideTitle && (
                  <header>
                    <h1 className={styles.docTitle}>{docsTitle}</h1>
                  </header>
                )}
